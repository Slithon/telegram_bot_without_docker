    ...
    try:
        ...
        if row:
            ...
            if db_version == VERSION:
                ...
                cursor.execute("DELETE FROM emergency_bot_subscribers")
                cursor.execute("""
                ALTER TABLE emergency_bot_subscribers
                ADD user_id VARCHAR(50),
                ADD CONSTRAINT user_id FOREIGN KEY (user_id) REFERENCES admins_2fa(user_id) ON DELETE CASCADE;
                """)
...
def add_moderator_standart(message):
    ...
    try:
        ...
        execute_db(
            "INSERT INTO pending_admins (moderator_id) VALUES (%s)",
            (str(first_moderator_id),),
            commit=True
        )
...
# ==================== Запуск бота ====================
print("Бот запущено БОТ РЕАЛЬНО ЗАПУСТИВСЯ Я В ШОЦІ")
